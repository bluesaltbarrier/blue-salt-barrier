#!/bin/bash
# 120 km ensemble run for Jan 2022, pristine baseline.
...
PIPELINE_LOG=/mpas/run_120km/pipeline_ensemble_2022_120km.log
echo "===== 120 km ensemble Jan 2022 pristine started $(date) =====" > "$PIPELINE_LOG"

# Year-specific pristine init
INIT_FILE=/opt/jan_120km/x1.40962.init.2022.pristine.nc
if [ ! -f "$INIT_FILE" ]; then
...
mpirun --allow-run-as-root --oversubscribe -np 12 ./atmosphere_model > log.salt.2022.out 2>&1
echo "  SALT finished at $(date)"
echo "[$(date)] SALT finished" >> "$PIPELINE_LOG"

HOST_DIR=/host/GITIGNORE/simulation_outputs/results_120km_jan_pohlker_l4_2022_prescribed_ccn_salt
mkdir -p "$HOST_DIR"
mv history.*.nc diag.*.nc "$HOST_DIR/" 2>/dev/null || true
cp log.atmosphere.0000.out log.salt.2022.out "$HOST_DIR/" 2>/dev/null || true
...
mpirun --allow-run-as-root --oversubscribe -np 12 ./atmosphere_model > log.nosalt.2022.out 2>&1
echo "  NOSALT finished at $(date)"
echo "[$(date)] NOSALT finished" >> "$PIPELINE_LOG"

HOST_DIR=/host/GITIGNORE/simulation_outputs/results_120km_jan_pohlker_l4_2022_prescribed_ccn_nosalt
mkdir -p "$HOST_DIR"
mv history.*.nc diag.*.nc "$HOST_DIR/" 2>/dev/null || true
cp log.atmosphere.0000.out log.nosalt.2022.out "$HOST_DIR/" 2>/dev/null || true

echo
echo "===== 120 km ensemble Jan 2022 complete $(date) ====="
