#!/usr/bin/env python3
"""
BL preservation diagnostic for the 120 km l=4 Pohlker pristine SALT run
WITH the prescribed-CCN patch applied.

Direct counterpart to bl_preservation_240km_prescribed_ccn.py.
Pass criterion: cloud-base nwfa (lev 5-8) within ±5% of init throughout.
"""
import os
import glob
import numpy as np
...
INIT_PRISTINE = "/opt/jan_120km/x1.40962.init.pristine.nc"
RUN_DIR = "/host/GITIGNORE/simulation_outputs/results_120km_jan_pohlker_l4_2025_prescribed_ccn_salt"
STATIC = "/opt/x1.40962.static.nc"
OUT_TXT = "/host/mpas_analysis/bl_preservation_120km_prescribed_ccn.txt"
OUT_PNG = "/host/mpas_analysis/bl_preservation_120km_prescribed_ccn.png"
...
print("\nInit pristine band means (raw pristine init, before SALT 2x perturbation):")
for name, sl in LEVEL_BANDS.items():
    val = band_mean_per_cm3(nwfa_init, sl)
    print(f"  {name:24s} {val:8.1f} /cm³")

print("\nExpected SALT-init band means (2x pristine over IGBP-2):")
init_salt_band = {}
for name, sl in LEVEL_BANDS.items():
    val = band_mean_per_cm3(nwfa_init, sl) * 2.0
...
with open(OUT_TXT, "w") as fh:
    fh.write("BL preservation diagnostic, 120 km l=4 Pohlker pristine SALT, Jan 2025\n")
    ...
    fh.write("\n")
    fh.write(txt + "\n")
    fh.write(f"\nPASS CRITERION (cloud-base lev 5-8 within ±{PASS_TOLERANCE*100:.0f}% of init): {'PASS' if cb_pass else 'FAIL'}\n")
...
try:
    ...
    title = "120 km l=4 Pohlker pristine SALT with PRESCRIBED-CCN PATCH"
